                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body" style="padding-top:0;">
                        ...
                        <div class="row no-print">
                            <!-- TODO: copy the line bellow -->
                            <div class="col-md-12 mDMb10">
                                <button type="button" class="btn btn-sm btn-warning collectSelected" id="load" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Please Wait.."><i class="fa fa-money"></i> <?php echo $this->lang->line('collect') . " Fees " . $this->lang->line('selected') ?></button>
                                <a href="#" class="btn btn-sm btn-info printSelected"><i class="fa fa-print"></i> <?php echo $this->lang->line('print') . " & " . $this->lang->line('collect') . " " . $this->lang->line('selected') ?> </a>
                                <?php
                                if (!empty($siblings)) {
                                ?>
                                <div class="btn-group">
                                    <button type="button" class="btn btn-sm btn-primary dropdown-toggle" data-toggle="dropdown" ><i class="fa fa-solid fa-group"></i>  &nbsp; Collectfee for siblings</button>
                                    <div class="dropdown-menu">
                                    <?php
                                    foreach ($siblings as $sibling_key => $sibling_value) {
                                    ?>
                                    <a class="dropdown-item btn-sm" href="<?php echo site_url('studentfee/addfee/' . $sibling_value->session_id) ?>"><?php echo $sibling_value->firstname . " " . $sibling_value->lastname ?></a>
                                    <?php
                                    }
                                    ?>
                                    </div>
                        ...
                        <div class="table-responsive">
                            ...
                            <table class="table table-striped table-bordered table-hover example table-fixed-header">
                                ...
                                <script>
                                    //console.log(<?php echo json_encode($student_due_fee) ?>);
                                </script>
                                <tbody>
                                    <?php
                                    $total_amount = 0;
                                    ...
                                    <?php
                                        ...
                                        foreach ($feesy as $fee_value) {
                                            ...
                                            ?>
                                            <script>
                                                //console.log(<?php echo json_encode($fee_value) ?>);
                                            </script>
                                            <?php
                                            ...
                                            if ($feetype_balance > 0 && strtotime($fee_value->due_date) < strtotime(date('Y-m-d'))) {
                                                ...
                                                <td>
                                                    <div class="btn-group pull-right">
                                                        <button type="button" data-student_session_id="<?php echo $fee_value->student_session_id; ?>" data-student_fees_master_id="<?php echo $fee_value->id; ?>" data-fee_groups_feetype_id="<?php echo $fee_value->fee_groups_feetype_id; ?>"
...
<div id="listCollectionAndPrintModal" class="modal fade">
    <div class="modal-dialog">
        <form action="<?php echo site_url('studentfee/addfeegrp'); ?>" method="POST" id="collect_print_fee_group">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                    <h4 class="modal-title"><?php echo $this->lang->line('collect') . " " . $this->lang->line('fees'); ?></h4>
                ...
                <div class="modal-footer">
                    <button type="submit" class="btn btn-primary payment_collect_print" data-action="print" data-loading-text="<i class='fa fa-spinner fa-spin '></i> Processing"><i class="fa fa-money"></i> <?php echo $this->lang->line('payandprint'); ?></button>
                </div>
            </div>
        </form>
    </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#listCollectionAndPrintModal').modal({
            backdrop: 'static',
            keyboard: false,
            show: false
        });
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.status === "success") {
                    ...
                    //console.log('-------------');
                    //console.log(data);
                    //console.log(fee_groups_feetype_id);
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $(document).on('click', '.printSelected', function () {
            ...
            if (array_to_collect_fees.length === 0) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        $("#listCollectionAndPrintModal .modal-body").html(data.view);
                        $(".date").datepicker({
                            format: date_format,
                            autoclose: true,
                            language: '<?php echo $language_name; ?>',
                        ...
                        $("#listCollectionAndPrintModal").modal('show');
                        $this.button('reset');
                    },
                    error: function (xhr) { // if error occured
                        alert("Error occured.please try again");
        ...
        $(document).on('click', '.payment_collect_print', function () {
            console.log('success');
        })
        $("#collect_print_fee_group").submit(function(event){
            var form = $(this);
                ...
                success: function (response) {
                    ...
                    if (response.status === 1) {
                        ...
                        if (array_to_print.length === 0) {
                            ...
                            console.log(array_to_print.length);
                            ...
                            $.ajax({
                                ...
                                success: function (response1) {
                                    console.log('printsuc')
                                    console.log(response1)
                                    Popup(response1, true);
                                }
                            });
                        }
        ...
        // $(document).on('click', '.payment_collect_print', function () {
        //     console.log('success');
        //     $("#collect_fee_group").submit(function(){
        //         console.log('success');
        //         var array_to_print = [];
        //         $.each($("input[name='fee_checkbox']:checked"), function () {
        //             var fee_session_group_id = $(this).data('fee_session_group_id');
