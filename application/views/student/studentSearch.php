                                            ...
                                            ?>
                                                ...
                                                ?>
                                                    <tr id=<?php echo $student['id'] ?>>
                                                        <script>
                                                            stud = <?php echo json_encode($student) ?>;
                                                            console.log(stud);
                                                            <?php
                                                            $danger = false;
                                                            $school_settings = $this->db->query('select session_id from sch_settings')->result();
                                                            $current_session_id = $school_settings[0]->session_id;
                                                            $student_current_session_id = $this->db->query('
                                                            select
                                                                id
                                                            from
                                                                student_session
                                                            ...
                                                            ')->result()[0]->id;
                                                            //$query = $this->db->query('select s.id, d.amount_detail from student_fees_deposite d join student_fees_master m on d.student_fees_master_id = m.id join student_session ss on m.student_session_id = ss.id join students s on ss.student_id = s.id where s.id =' . $student['id'] . ' and month(d.created_at) = month(curdate()) and year(d.created_at) = year(curdate())');
                                                            $query = $this->db->query('
                                                                SELECT
                                                                    `student_fees_master`.*,
                                                                    fee_groups.name
                                                                ...
                                                                ',
                                                                array($student_current_session_id)
                                                                //     array($student['id'])
                                                            );
                                                            ?>
                                                                console.log(<?php echo json_encode($query->result()) ?>);
                                                            <?php
                                                            foreach($query->result() as $r){
                                                                $danger = false;
                                                                ?>
                                                                //console.log(<?php echo json_encode($r->fee_session_group_id) ?>);
                                                                <?php
                                                            $query2 = $this->db->query(
                                                                ...
                                                                SELECT
                                                                    ...
                                                                    student_fees_deposite.amount_detail as `amount_detail`
                                                                FROM
                                                                    `student_fees_master`
                                                                ...
                                                                WHERE
                                                                    ...
                                                                    and student_fees_master.id = ?
                                                                    and fee_groups_feetype.due_date <= curdate()
                                                                    and (student_fees_deposite.id is null
                                                                    or student_fees_deposite.amount_detail is null)
                                                                order by
                                                                    fee_groups_feetype.due_date ASC;
                                                                ',
                                                                array($r->fee_session_group_id,$r->id)
                                                            );
                                                            ...
                                                            ?>
                                                            b = <?php echo json_encode($danger) ?>;
                                                            if (b) {
                                                                paid.push(false);
                                                                let row = document.getElementById(<?php echo $student['id'] ?>);
