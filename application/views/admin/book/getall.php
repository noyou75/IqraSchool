<script>
    ...
    fees.forEach((d, i, arr) => {
        let discount = 0;
        // let fine = 0;
        let amount_detail = JSON.parse(d.collected_by);
        let description = "";
        Object.keys(amount_detail).forEach(k=>{
            discount += parseFloat(amount_detail[k].amount_discount);
            // fine += parseFloat(amount_detail[k].amount_fine);
            description = amount_detail[k].description;
        });
        ...
        financial_data.push({
            ...
            name: d.full_name,
            category:d.category,
            // amount: d.amount - parseFloat(JSON.parse(d.collected_by)['1'].amount_discount),
            amount: d.amount - discount,
            // collectedBy: JSON.parse(d.collected_by)['1'].description,
            collectedBy: description,
            date_data: new Date(d.date),
            date: new Date(d.date).toDateString(),
            color: "info"
        });
        // if (d.amount !== null) sum += parseFloat(d.amount) - parseFloat(JSON.parse(d.collected_by)['1'].amount_discount);
        if (d.amount !== null) sum += parseFloat(d.amount) - discount;
    });
...
<script>
    ...
    if(
        `<?php
            ...
            foreach ($userdata['roles'] as $ckey => $cvalue) {
                echo $cvalue;
            }
        ?>` == 7
    ) {
