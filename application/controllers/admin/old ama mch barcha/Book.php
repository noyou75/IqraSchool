...
class Book extends Admin_Controller
{
    ...
    public function getall()
    {
        ...
        if ($this->input->method() == "get") {
            ...
            $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE date=CURRENT_DATE();')->result();
            $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE date=CURRENT_DATE();')->result();
            $data['fees'] = $this->db->query('
                SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
            ...
            switch ($period) {
                ...
                case 'this_week':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE week(date)=week(CURRENT_DATE());')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE week(date)=week(CURRENT_DATE());')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                        ...
                        right JOIN student_session on student_fees_master.student_session_id = student_session.id
                        right join students on student_session.student_id = students.id
                        where week(DATE(student_fees_deposite.created_at)) = week(CURRENT_DATE());
                    ')->result();
                    $data['method'] = $this->input->method();
                    break;

                case 'last_week':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE week(date)=week(CURRENT_DATE())-1;')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE week(date)=week(CURRENT_DATE())-1;')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                ...
                case 'this_month':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE month(date)=month(CURRENT_DATE());')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE month(date)=month(CURRENT_DATE());')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                ...
                case 'last_month':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE month(date)=month(CURRENT_DATE()) - 1;')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE month(date)=month(CURRENT_DATE()) - 1;')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                ...
                case 'last_3_month':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 3 );')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 3 );')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                        ...
                        right JOIN student_session on student_fees_master.student_session_id = student_session.id
                        right join students on student_session.student_id = students.id
                        where (month(DATE(student_fees_deposite.created_at)) <= month(CURRENT_DATE()) - 1 AND month(DATE(student_fees_deposite.created_at)) >= month(CURRENT_DATE()) - 3);
                    ')->result();
                    $data['method'] = $this->input->method();
                    break;

                case 'last_6_month':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 6 );')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 6 );')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                        ...
                        right JOIN student_session on student_fees_master.student_session_id = student_session.id
                        right join students on student_session.student_id = students.id
                        where (month(DATE(student_fees_deposite.created_at)) <= month(CURRENT_DATE()) - 1 AND month(DATE(student_fees_deposite.created_at)) >= month(CURRENT_DATE()) - 6);
                    ')->result();
                    $data['method'] = $this->input->method();
                    break;

                case 'last_12_month':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 12 );')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 12 );')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                        ...
                        right JOIN student_session on student_fees_master.student_session_id = student_session.id
                        right join students on student_session.student_id = students.id
                        where (month(DATE(student_fees_deposite.created_at)) <= month(CURRENT_DATE()) - 1 AND month(DATE(student_fees_deposite.created_at)) >= month(CURRENT_DATE()) - 12);
                    ')->result();
                    $data['method'] = $this->input->method();
                    break;

                case 'this_year':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE year(date)=year(CURRENT_DATE());')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE year(date)=year(CURRENT_DATE());')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                ...
                case 'last_year':
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE year(date)=year(CURRENT_DATE()) - 1;')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE year(date)=year(CURRENT_DATE()) - 1;')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
                ...
                default:
                    ...
                    $data['expenses'] = $this->db->query('SELECT name, amount, date FROM `expenses` WHERE date BETWEEN STR_TO_DATE("'.$date_from.'","%d/%m/%Y") AND STR_TO_DATE("'.$date_to.'","%d/%m/%Y");')->result();
                    $data['income'] = $this->db->query('SELECT name, amount, date FROM `income` WHERE date BETWEEN STR_TO_DATE("'.$date_from.'","%d/%m/%Y") AND STR_TO_DATE("'.$date_to.'","%d/%m/%Y");')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount, student_fees_deposite.created_at as date 
                        FROM `student_fees_deposite`
                        LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
