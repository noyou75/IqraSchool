...
right join students on student_session.student_id = students.id
                ...
                case 'this_week':
                    $data['expenses'] = $this->db->query('SELECT expenses.name, expenses.amount, expense_head.exp_category, expenses.date
FROM `expenses`
join expense_head on expenses.exp_head_id = expense_head.id WHERE week(date)=week(CURRENT_DATE());')->result();
                    $data['income'] = $this->db->query('SELECT income.name, income.amount, income_head.income_category, income.date
FROM `income`
join income_head on income.inc_head_id = income_head.id WHERE week(date)=week(CURRENT_DATE());')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount,student_fees_deposite.amount_detail as collected_by, fee_groups.name as category, student_fees_deposite.created_at as date 
FROM `student_fees_deposite`
LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
...
right join students on student_session.student_id = students.id
                ...
                case 'last_week':
                    $data['expenses'] = $this->db->query('SELECT expenses.name, expenses.amount, expense_head.exp_category, expenses.date
FROM `expenses`
join expense_head on expenses.exp_head_id = expense_head.id WHERE week(date)=week(CURRENT_DATE())-1;')->result();
                    $data['income'] = $this->db->query('SELECT income.name, income.amount, income_head.income_category, income.date
FROM `income`
join income_head on income.inc_head_id = income_head.id WHERE week(date)=week(CURRENT_DATE())-1;')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount,student_fees_deposite.amount_detail as collected_by, fee_groups.name as category, student_fees_deposite.created_at as date 
FROM `student_fees_deposite`
LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
...
right join students on student_session.student_id = students.id
                ...
                case 'this_month':
                    $data['expenses'] = $this->db->query('SELECT expenses.name, expenses.amount, expense_head.exp_category, expenses.date
FROM `expenses`
join expense_head on expenses.exp_head_id = expense_head.id WHERE month(date)=month(CURRENT_DATE());')->result();
                    $data['income'] = $this->db->query('SELECT income.name, income.amount, income_head.income_category, income.date
FROM `income`
join income_head on income.inc_head_id = income_head.id WHERE month(date)=month(CURRENT_DATE());')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount,student_fees_deposite.amount_detail as collected_by, fee_groups.name as category, student_fees_deposite.created_at as date 
FROM `student_fees_deposite`
LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
...
right join students on student_session.student_id = students.id
                ...
                case 'last_month':
                    $data['expenses'] = $this->db->query('SELECT expenses.name, expenses.amount, expense_head.exp_category, expenses.date
FROM `expenses`
join expense_head on expenses.exp_head_id = expense_head.id WHERE month(date)=month(CURRENT_DATE()) - 1;')->result();
                    $data['income'] = $this->db->query('SELECT income.name, income.amount, income_head.income_category, income.date
FROM `income`
join income_head on income.inc_head_id = income_head.id WHERE month(date)=month(CURRENT_DATE()) - 1;')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount,student_fees_deposite.amount_detail as collected_by, fee_groups.name as category, student_fees_deposite.created_at as date 
FROM `student_fees_deposite`
LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
...
right join students on student_session.student_id = students.id
                ...
                case 'last_3_month':
                    $data['expenses'] = $this->db->query('SELECT expenses.name, expenses.amount, expense_head.exp_category, expenses.date
FROM `expenses`
join expense_head on expenses.exp_head_id = expense_head.id WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 3 );')->result();
                    $data['income'] = $this->db->query('SELECT income.name, income.amount, income_head.income_category, income.date
FROM `income`
join income_head on income.inc_head_id = income_head.id WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 3 );')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount,student_fees_deposite.amount_detail as collected_by, fee_groups.name as category, student_fees_deposite.created_at as date 
FROM `student_fees_deposite`
LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
...
right join students on student_session.student_id = students.id
                ...
                case 'last_6_month':
                    $data['expenses'] = $this->db->query('SELECT expenses.name, expenses.amount, expense_head.exp_category, expenses.date
FROM `expenses`
join expense_head on expenses.exp_head_id = expense_head.id WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 6 );')->result();
                    $data['income'] = $this->db->query('SELECT income.name, income.amount, income_head.income_category, income.date
FROM `income`
join income_head on income.inc_head_id = income_head.id WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 6 );')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount,student_fees_deposite.amount_detail as collected_by, fee_groups.name as category, student_fees_deposite.created_at as date 
FROM `student_fees_deposite`
LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
...
right join students on student_session.student_id = students.id
                ...
                case 'last_12_month':
                    $data['expenses'] = $this->db->query('SELECT expenses.name, expenses.amount, expense_head.exp_category, expenses.date
FROM `expenses`
join expense_head on expenses.exp_head_id = expense_head.id WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 12 );')->result();
                    $data['income'] = $this->db->query('SELECT income.name, income.amount, income_head.income_category, income.date
FROM `income`
join income_head on income.inc_head_id = income_head.id WHERE (month(date)<=month(CURRENT_DATE()) - 1 AND month(date) >= month(CURRENT_DATE()) - 12 );')->result();
                    $data['fees'] = $this->db->query('
                        SELECT CONCAT(students.firstname," ",students.lastname) as full_name, fee_groups_feetype.amount,student_fees_deposite.amount_detail as collected_by, fee_groups.name as category, student_fees_deposite.created_at as date 
FROM `student_fees_deposite`
LEFT JOIN fee_groups_feetype on student_fees_deposite.fee_groups_feetype_id = fee_groups_feetype.id
